...
const read_file = (path) => fs.readFileSync(path, 'utf8');
const write_file = (path, content) => fs.writeFileSync(path, content, 'utf8');

let html_content = read_file(INDEX_PATH);

// Extract parts
const head_match = html_content.match(/(<!DOCTYPE html>[\s\S]*?<\/head>)/);
let head = head_match ? head_match[1] : '';

// Replace nav links in the new pages to point to .html files, and add back to home
const nav_start = html_content.indexOf('<nav id="navbar">');
const nav_end = html_content.indexOf('</nav>') + 6;
let nav = html_content.substring(nav_start, nav_end);

// Modify nav links
nav = nav.replace(/href="#manifesto"/g, 'href="./manifesto.html"');
nav = nav.replace(/href="#kriteria"/g, 'href="./kriteria.html"');
nav = nav.replace(/href="#komisi"/g, 'href="./komisi.html"');
nav = nav.replace(/href="#panel"/g, 'href="./panel.html"');
nav = nav.replace(/href="#faq"/g, 'href="./faq.html"');
nav = nav.replace(/href="#gabung"/g, 'href="./gabung.html"');

// Add "Beranda" to nav links
const nav_links_marker = '<div class="nav-links">';
nav = nav.replace(nav_links_marker, nav_links_marker + '\n      <a href="./index.html">← Beranda</a>');

const footer_start = html_content.indexOf('<footer>');
const script_start = html_content.indexOf('<!-- SCRIPTS -->');
const footer = html_content.substring(footer_start, script_start);
const scripts = html_content.substring(script_start);
...
function build_page(title, content) {
    let page_head = head.replace('<title>parlemenbayangan.id — mengawal kabinet bayangan, menguji gagasan</title>', `<title>${title} — parlemenbayangan.id</title>`);
    return `${page_head}
<body>
${nav}
${content}
...
// Update index.html
html_content = html_content.replace(/href="#manifesto"/g, 'href="./manifesto.html"');
html_content = html_content.replace(/href="#kriteria"/g, 'href="./kriteria.html"');
html_content = html_content.replace(/href="#komisi"/g, 'href="./komisi.html"');
html_content = html_content.replace(/href="#panel"/g, 'href="./panel.html"');
html_content = html_content.replace(/href="#faq"/g, 'href="./faq.html"');
html_content = html_content.replace(/href="#gabung"/g, 'href="./gabung.html"');
...
console.log("All files generated and index.html updated successfully.");
